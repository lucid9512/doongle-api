...
## 2026-06-14 — Kafka env 이름 워커와 통일 (KAFKA_BROKER/KAFKA_TOPIC), 토픽 image-jobs
- 결정: api 의 Kafka 환경변수 이름을 워커(doongle-ai)와 통일한다. `KAFKA_BOOTSTRAP_SERVERS` → `KAFKA_BROKER`, `KAFKA_UPLOAD_TOPIC`(기본 "image-upload") → `KAFKA_TOPIC`(기본 "image-jobs"). api 코드만 수정(워커는 이미 이 이름을 쓴다).
- 이유: api 와 워커가 각각 개발되며 같은 브로커·토픽을 가리키는데 env 이름이 갈렸다. k8s ConfigMap 에서 **단일 값으로 양쪽에 주입**하려면 이름이 같아야 한다. 워커 스타일을 채택 — 더 짧고, 토픽이 단일(`image-jobs`)이라 "UPLOAD" 접두사는 부정확(업로드 외 의미도 포괄).
- 변경 범위: `config.py`(필드 2개), `kafka.py`(producer 1 + 로그 2 + docstring), `services/job.py`(produce 2곳), `.env`·`.env.example`. 인터페이스/처리 로직은 불변(이름만 치환).
- 검증: `grep KAFKA_BOOTSTRAP_SERVERS|KAFKA_UPLOAD_TOPIC` 결과 코드/설정 파일엔 잔존 없음(docs/TASKS.md 의 과거 단계 기록만 남김 — 당시 사실 기록이라 보존). 로컬 Kafka(localhost:9092) 기동 후 `uvicorn` 으로 producer 정상 시작 로그 확인.
- 비고: 검증 중 `.env` 의 `MINIO_SECURE=falseㅇㅇ` 오타(부팅 실패 유발)를 `false` 로 바로잡음(.env 는 로컬 전용/gitignore).