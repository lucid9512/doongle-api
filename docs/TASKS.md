...
- [x] 단계 5 — 업로드 API
  ...
  - (후속) 로직을 `app/services/job.py`의 `JobService`로 이전, 라우터는 얇게 (서비스 계층 컨벤션 정합)
...
- [x] 단계 6 — Job 조회 API
  ...
  - (후속) 조회 로직을 `JobService`로 이전, 없는 job은 도메인 예외(`ObjectNotFoundError`)로 404
  - (후속) `GET /jobs/{id}/image` 제거 — 1회성 UI라 미리보기는 클라이언트 로컬 File로 렌더 (DECISIONS 참고)
...
- [x] 단계 7 — Jinja 업로드 + 폴링 화면
  ...
  - 썸네일은 업로드한 로컬 File을 `URL.createObjectURL`로 렌더 (서버 재요청 없음)
...
- [x] 단계 8 — 로컬 검증
  - `docker-compose.yaml`에 Kafka(KRaft 단일노드, 이중 리스너 localhost:9092 + kafka:29092) + kafka-ui(:8080) 추가
  - 로컬 `.env`의 `KAFKA_BOOTSTRAP_SERVERS=localhost:9092`로 맞춤(.env는 gitignore)
  - `docker compose up -d` → uvicorn 기동 시 producer가 localhost:9092 연결 로그 확인
  - 검증 결과(이미지 2장 업로드):
    - 업로드 응답: job 2건 status=pending, `GET /jobs/{id}` 폴링도 pending
    - `./uploads/`에 png 2개 저장
    - DB `jobs`에 pending row 2개(image_path=uploads/...)
    - Kafka `image-upload` 토픽에 `{job_id, image_path}` 메시지 2건
    - `GET /jobs/{id}/image` → 200 image/png, `/upload-ui` → 200
  - (워커가 없으므로 pending에서 멈춤이 정상 — 의도된 동작)

- [x] (후속) MinIO 스토리지 백엔드 추가
  - `app/core/storage.py`: `MinioStorage(StorageBackend)` — minio 공식 SDK, `save()`는 오브젝트 키만 반환(버킷은 env 관리), `load()`는 `get_object`. 클라이언트는 `__init__`에서 1회 생성, 버킷 존재 가정
  - `app/deps/storage.py`: `STORAGE_BACKEND=minio` 분기 추가
  - `config.py`에 `MINIO_ENDPOINT/ACCESS_KEY/SECRET_KEY/BUCKET/SECURE` + `.env.example`
  - `pyproject.toml`에 `minio` 의존성 추가
  - `StorageBackend` 인터페이스·`LocalStorage`·`JobService`·업로드 라우터는 미변경(인터페이스로만 의존)
  - 설계 결정은 DECISIONS.md 참고(키만 반환 / boto3 아닌 minio SDK / 버킷 존재 가정 / 동기 블로킹 trade-off)

- [x] (후속) api Dockerfile + .dockerignore
  - `Dockerfile`: `python:3.13-slim`(추론 없음→경량), `--platform=linux/amd64` 고정, `PYTHONUNBUFFERED=1`
  - 의존성: 로컬 `poetry export`로 뽑은 `requirements.txt` 먼저 COPY/설치(레이어 캐시) → 앱 소스·`alembic.ini`·`migrations/` COPY
  - 설정값은 ENV 미하드코딩(런타임 k8s ConfigMap/Secret 주입), `EXPOSE 8000`
  - CMD(셸): `alembic upgrade head && uvicorn app.main:app --host 0.0.0.0 --port 8000`
  - `.dockerignore`: .venv/.env/.git/uploads/logs/docs/`*.md`/__pycache__ 등 제외
  - 설계 결정은 DECISIONS.md 참고(슬림 베이스 / amd64 고정 / 시작 시 마이그레이션 + 다중 레플리카 시 Job 분리 여지)