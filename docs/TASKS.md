...
- [ ] 단계 4 — Job 모델 + 마이그레이션
  - `app/models/job.py`: `Job` (job_id, filename, image_path, status="pending", result nullable, created_at, updated_at)
  - `app/models/__init__.py` 등록
  - alembic revision --autogenerate -m "add jobs table" → upgrade head
